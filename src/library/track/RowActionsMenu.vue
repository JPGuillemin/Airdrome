// RowActionsMenu.vue
<template>
  <OverflowMenu ref="menuRef" external>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      // Exposé pour que la ligne parente (TrackList) puisse ouvrir ce
      // menu à un point donné : coordonnées du clic droit, ou rect de la
      // ligne pour le long-press.
      const openAt = (point: { top: number; bottom: number; left: number; right: number }) => {
        menuRef.value?.openAt(point)
      }
